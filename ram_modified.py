...
def calc_reward(outputs):
    ...
    logits = tf.matmul(outputs, Wa_h_a) + Ba_h_a 
    #tf.nn.xw_plus_b(outputs, Wa_h_a, Ba_h_a)
    softmax = tf.nn.softmax(logits)  #(p_y)
    pred_labels = tf.argmax(logits, 1) #(max_p_y)
    labels_ph = tf.cast(labels_placeholder, tf.int64) # correct_y
    
    #Triplet loss
    #xent = batch_hard_triplet_loss(labels=labels_ph, embeddings=outputs, margin=0.5, squared=False)
    ...
    # reward for all examples in the batch #R rewards
    R = tf.cast(tf.equal(pred_labels, labels_ph), tf.float32)
    ...
    #R = tf.cast(tf.equal(pred_labels, labels_ph), tf.float32)
    #R = tf.expand_dims(R, 1)  # [batch_sz, 1]
    #R = tf.tile(R, (1, nGlimpses))  # [batch_sz, timesteps]
    
    
    
   # R = tf.reduce_mean(R) # mean reward
   # R = tf.reshape(R, (batch_size, 1))
   # R = tf.tile(R, [1, (nGlimpses)*2])
    
    
    
    logll = gaussian_pdf(mean_locs, sampled_locs)
    logll = tf.reshape(logll, (batch_size, (nGlimpses) * 2))
    
    #logll = loglikelihood(mean_locs, sampled_locs, config.loc_std)
    #logll = tf.reshape(logll, (batch_size, (nGlimpses) * 2))
    
    advs = R - tf.stop_gradient(b) # MLE for regressing the optimal μ as parameter input to stochastic network
    logllratio = tf.reduce_mean(logll * advs)
    

    baselines_mse = tf.reduce_mean(tf.square((R - b)))
    var_list = tf.trainable_variables()
# hybrid loss   optimizing only our differentiable network with following 3 targets with equal weights:
...
# learning rate
    ...
    training_steps_per_epoch = dataset.train.num_examples // batch_size
...
# decay per training epoch
    ...
    return loss, reward, pred_labels, labels_ph, train_op, b, tf.reduce_mean(b), tf.reduce_mean(R - b), learning_rate
